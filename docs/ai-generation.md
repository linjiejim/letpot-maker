...
2. `sculpture` can compose a new printable subject from ten allowlisted closed-solid nodes: ellipsoid, rounded box, cylinder, cone, capsule, torus, pitched roof, leaf, star and heart. Nodes support bounded position, size and rotation, three semantic colors, mirror symmetry, additive geometry and up to five shallow decorative cutters. Each additive node attaches to the locked core or an earlier node, and the compiler inserts a printable fusion bridge from inside the parent solid.

Every sculpture recipe is normalized to a 25–50 mm topper height, 20–40 mm topper width, no more than 24 declared nodes, normalized coordinates and feature sizes, three hex colors, and a surface-style flag. Unsupported kinds and fields are discarded, out-of-range numbers are clamped, duplicate IDs are repaired, forward or unknown attachment references fall back to the core, and arbitrary code is never evaluated.
